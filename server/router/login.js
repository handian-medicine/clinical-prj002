...
router.post('/',function (req, res, next) {
    ...
    // 4. 必要信息存入cookie   把body里的token信息存入cookie,后续再向服务器发送请求就不必每次把token写到请求里了
          ...
          // res.cookie("prj002token", usertoken, {maxAge: 1000 * 60 * 60 * 8, httpOnly: true})
          res.cookie("usertoken", usertoken, {maxAge: 1000 * 60 * 60 * 8, httpOnly: true})
          res.cookie("loginInfo", {"email": req.body.email,"password": req.body.password}, {maxAge: 1000 * 60 * 60 * 8, httpOnly: true})
    ...
    // 6. 向vue返回登录失败信息
          let user = {'email':lreq.body.email, 'password':req.body.password}
