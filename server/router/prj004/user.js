...
router.post('/search', function (req, res, next) {
  const options = {
    url: myConst.apiurl + "/prj004/search/",
    form: req.body.search,//浏览器发送过来的req的body  和  后端返回的response的body格式类型不一样?
    qs: {page:req.body.page},
    headers: {'Authorization': 'Bearer ' + req.cookies.prj004token.access_token}
  }
...
router.post('/export', function (req, res, next) {
  const options = {
    url: myConst.apiurl + "/prj004/fileout/",
    headers: {'Authorization': 'Bearer ' + req.cookies.prj004token.access_token}
  }
...
router.post('/add', function (req, res, next) {
  ...
  const options = {
    url: myConst.apiurl + "/prj004/info/",
...
router.post('/check', function (req, res, next){
  var options = {
        ...
        form:req.body,
        headers: {
            'Authorization': 'Bearer ' + req.cookies.prj004token.access_token
        }
      }
...
router.post('/form', function(req, res, next) {
  ...
  var options = {
    url: myConst.apiurl + "/prj004/"+ req.body.formName + "/",
...
router.get('/echarts', function (req, res, next) {
    var options = {
        url: myConst.apiurl + "/prj004/charts/",
        headers: {
            'Authorization': 'Bearer ' + req.cookies.prj004token.access_token
        }
    }
