<template>
  <el-dialog title="专病情况" class="my-dialog disease-dialog"
            :visible.sync="dialogVisible"
            :close-on-click-modal="false" width="100%" center
            @close='resetDialog'>
    <el-form ref="diseaseForm" :model="diseaseForm" label-width="80px" label-position="left">
      ...
      <h4>1. 多毛评分</h4>
        <table border="1" cellspacing="0" cellpadding="3px" style="width: 80%;margin:auto">
          ...
          <tr v-for="(val, key) in hairy_table" :key="key">
            ...
            <td v-for="(item, index) in val.list" :key="item">
              <el-radio v-model="diseaseForm[key]" :label="index" @change="getHairyValue($event,key)">{{item}}</el-radio>
            </td>
            <td>{{val.score}}</td>
          </tr>
      ...
      <h4>2. 女性型脱发Ludwig分型 <el-switch v-model="diseaseForm.is_tuofa" active-text="有" inactive-text="无"></el-switch></h4>
        <div v-show="diseaseForm.is_tuofa">
          <p><el-radio v-model="diseaseForm.tuofa" label="1">Ⅰ型：头顶部毛发弥漫性稀少，毛发变细，头皮稍暴露</el-radio></p>
          <p><el-radio v-model="diseaseForm.tuofa" label="2">Ⅱ型；头顶＋前头部毛发稀少，毛发变纤细，头皮裸露稍明显</el-radio></p>
          <p><el-radio v-model="diseaseForm.tuofa" label="3">Ⅲ型：前头部＋头顶部头发明显稀疏，头发纤细，但发际线仍保留</el-radio></p>
        </div>
        ...
        <table border="1" cellspacing="0" cellpadding="3px" style="width: 85%;margin:auto">
          ...
          <tr v-for="(val, key) in depression_table" :key="key">
            ...
            <td v-for="(item, index) in val.list" :key="index+20" style="text-align:center">
              <el-radio v-model="diseaseForm[key]" :label="index" @change="getDepressionValue($event,key)">{{item}}</el-radio>
            </td>
            <td>{{val.score}}</td>
          </tr>
          <tr style="height:50px;">
      ...
      <h4>6. 焦虑自评</h4>
        <table border="1" cellspacing="0" cellpadding="3px" style="width: 85%;margin:auto">
          ...
          <tr v-for="(val, key) in anxiety_table" :key="key">
            <td style="text-align:left">{{val.name}}</td>
            <td v-for="(item, index) in val.list" :key="index+20" style="text-align:center">
              <el-radio v-model="diseaseForm[key]" :label="index" @change="getAnxietyValue($event,key)">{{item}}</el-radio>
            </td>
            <td>{{val.score}}</td>
          </tr>
          <tr style="height:50px;">
    ...
    <span slot="footer">
      <el-button type="primary" :disabled="check_status=='审核通过'" v-if="exist"  @click="updateDiseaseForm">确定</el-button>
      <el-button type="primary" v-else  @click="createDiseaseForm">确定</el-button>
...
export default {
  ...
  data() {
    return {
      hairy_table:{
        "chun":     {name:"上唇",list:["无毛","散在几根","外侧1/4","外侧1/2","全部上唇"],score:0},
        "xiae":     {name:"下额",list:["无毛","散在几根","散在，较多","覆盖下额，较多","覆盖下额，密"],score:0},
        "xiongbu":  {name:"胸部",list:["无毛","乳晕周围","乳晕，胸中线","胸中线与乳晕相连","覆盖前胸"],score:0},
        "shangfu":  {name:"上腹",list:["无毛","中线几根","中线，较多","覆盖上腹1/2","覆盖上腹全部"],score:0},
        "xiafu":    {name:"下腹",list:["无毛","中线几根","中线，细带状","中线，粗带状","与阴毛相连，菱形"],score:0},
        "shangtun": {name:"上臀",list:["无毛","内侧几根","覆盖内侧面1/2","覆盖内侧面，稀","覆盖内侧面，密"],score:0},
        "datui":    {name:"大腿",list:["无毛","内侧几根","覆盖内侧面1/3","覆盖内侧面，稀","覆盖内侧面，密"],score:0},
        "shangbei": {name:"上背",list:["无毛","散在几根","覆盖上背部1/2","覆盖上背，稀","覆盖上背，密"],score:0},
        "yao":      {name:"腰骶",list:["无毛","骶部几根","覆盖腰骶部1/2","覆盖腰骶部3/4","覆盖整个腰骶部"],score:0},
      },
      cuochuang_table:{
  ...
  computed:{
    hairy_score () {
      ...
      for (var item in this.hairy_table) {
        score += this.hairy_table[item].score
      }
    ...
    depression_score () {
      ...
      for (var item in this.depression_table) {
        score += this.depression_table[item].score
      }
    ...
    anxiety_score () {
      ...
      for (var item in this.anxiety_table) {
        score += this.anxiety_table[item].score
      }
  ...
  methods: {
    ...
    getDepressionValue (e,key) {
      this.depression_table[key].score = e
    },
    getAnxietyValue (e,key) {
      this.anxiety_table[key].score = e
    },
    updateDiseaseForm () {
      apiUpdatePatientDataForm({formData:this.diseaseForm,formName:this.formName})
      .then((res)=> {
        this.resetDialog()
        if (res.data.detail) {
    ...
    createDiseaseForm () {
      apiCreatePatientDataForm({formData:this.diseaseForm,formName:this.formName})
      .then((res)=> {
        this.resetDialog()
        if (res.data.detail) {
          this.$message({message: '对不起, 您没有对该记录操作的权限',type: 'error'})
  ...
  created() {
    this.$on("openEvent", (data)=>{
      ...
      if (!data.exist) {
        //未创建
        this.diseaseForm.info = data.formData.info
      } else {
        //已创建(修改)
        this.diseaseForm = data.formData
      }
    })
  }
