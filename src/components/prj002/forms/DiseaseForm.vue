...
export default {
  ...
  methods: {
    ...
    updateDiseaseForm () {
      console.log("看这里",this.diseaseForm)
  ...
  created() {
    this.$on("openEvent", (data)=>{
      ...
      if (!data.exist) {
        ...
        //这里this.diseaseForm[item]返回的是数字，不需要转换
        for (var item in this.hairy_table){this.hairy_table[item].score = this.diseaseForm[item]}

        //这里this.diseaseForm[item]返回的是字符串，需要转换
        var list = {"没有或很少时候":"0","小部分时候":"1","相当多时候":"2","绝大部分或全部时候":"3"}
        for (var item in this.depression_table){
          if(this.diseaseForm[item] == null || this.diseaseForm[item] == undefined || this.diseaseForm[item] == ''){
            this.depression_table[item].score = 0
          } else {
            this.depression_table[item].score = Number(list[this.diseaseForm[item]])
          }
        }
        for (var item in this.anxiety_table){
          if(this.diseaseForm[item] == null || this.diseaseForm[item] == undefined || this.diseaseForm[item] == ''){
            this.anxiety_table[item].score = 0
          } else {
            if (this.anxiety_table[item].reverse){
              this.anxiety_table[item].score = 3 - Number(list[this.diseaseForm[item]])
            } else {
              this.anxiety_table[item].score = Number(list[this.diseaseForm[item]])
            }
          }
        }
